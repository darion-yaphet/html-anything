  ...
  constructor(public readonly agent: string, public readonly protocol: string) {
    super(
      `${agent} uses the ${protocol} protocol, which is not yet wired up in this build. ` +
        `Pick one of: claude / codex / cursor-agent / gemini / copilot / opencode / qwen / qoder / codewhale / deepseek-tui / aider / antigravity.`,
    );
  }
}
...
function parseLineWithState(agent: string, line: string, state: ParseState): AgentParse[] {
  ...
  // These adapters emit plain text on stdout rather than JSON envelopes.
  // DeepSeek tool calls go to stderr, which is forwarded as `stderr` events.
  if (agent === "aider" || agent === "codewhale" || agent === "deepseek-tui" || agent === "antigravity") {
